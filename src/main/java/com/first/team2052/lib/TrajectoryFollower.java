...
public class TrajectoryFollower {
    public static class TrajectoryConfig {
        ...
        public String toString() {
            return "dt: " + dt + ", max_acc: " + max_acc + ", max_vel: " + max_vel;
        }
    }
    ...
    public double calculate(double position) {
        ...
        if (isFinishedTrajectory()) {
            ...
            double max_reachable_velocity_disc = cur_vel2 / 2.0 + config_.max_acc * distance_to_go;
            ...
            // to
            // cruise_vel
            double x_start = cur_vel * t_start + .5 * config_.max_acc * t_start * t_start;
            double t_end = Math.abs(cruise_vel / config_.max_acc); // Decelerate
            // to zero
            // vel.
            double x_end = cruise_vel * t_end - .5 * config_.max_acc * t_end * t_end;
            ...
            if (t_start >= dt) {
                next_state_.pos = cur_vel * dt + .5 * config_.max_acc * dt * dt;
                ...
                next_state_.pos = x_start + x_cruise + cruise_vel * delta_t - .5 * config_.max_acc * delta_t * delta_t;
        ...
        double output = kp_ * error + kd_ * ((error - last_error_) / dt - setpoint_.vel)
