...
public class ReneeAuto extends AutoMode {
    ...
    protected void init() throws AutoModeEndedException {
        ...
        ForwardPath.add(new Path.Waypoint(new Translation2d(40, 0), 30));
        ForwardPath.add(new Path.Waypoint(new Translation2d(0,40), 30, "Start Shooting"));
        ForwardPath.add(new Path.Waypoint(new Translation2d(-40,0), 30));
        ForwardPath.add(new Path.Waypoint(new Translation2d(0,-40),30, "Drop Gear"));

        runAction(new SeriesAction(Arrays.asList(new FollowPathAction(new Path(ForwardPath), false))));
        runAction(new SeriesAction(Arrays.asList(new WaitForPathMarkerAction("Start Shooting"),new StartShootingAction())));
        runAction(new SeriesAction(Arrays.asList(new WaitForPathMarkerAction("Drop Gear"), new DropGearAction())));
    }
}
